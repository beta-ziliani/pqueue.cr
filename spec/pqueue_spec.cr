...
describe PQueue::PQueue do
  it "correctly performs insertions" do
    ...
    a = pqueue.to_a
    (1..1000).each do |i|
      a[i - 1] == {i, i} || raise "{#{i}, #{i}} dissappeared"
    end
  end

  it "correctly performs deletions" do
    pqueue = PQueue::PQueue(Int32, Int32).new(10, 0, Int32::MAX, 0)
  ...
  it "correctly performs parallel deletions" do
    ...
    j = 0
    while j < a.size
      a[j] == {i, i} || raise "{#{i}, #{i}} dissappeared"
      j += 1
      i += 1
    end
  end

  pending "Parallel insertions and deletions" do
    pqueue = PQueue::PQueue(Int32, Int32).new(10, 0, Int32::MAX, 0)
    ...
    (0...8).each do |i|
      if i % 2 == 0
        ...
        spawn do
          # wait a bit to let the other coroutine insert some elements
          sleep 1.millisecond
          (0...100).each do
            t = pqueue.deletemin
            ch.send t
